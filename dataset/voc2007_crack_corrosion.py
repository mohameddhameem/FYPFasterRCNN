...
class VOC2007CrackCorrosion(Base):
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        if self._mode == VOC2007CrackCorrosion.Mode.TRAIN:
            path_to_image_ids_txt = os.path.join(path_to_imagesets_main_dir, 'trainval.txt')
        elif self._mode == VOC2007CrackCorrosion.Mode.EVAL:
        ...
        for image_id in image_ids:
            ...
            annotation = VOC2007CrackCorrosion.Annotation(
                filename=root.find('filename').text,
                objects=[VOC2007CrackCorrosion.Annotation.Object(
            ...
            annotation.objects = [obj for obj in annotation.objects if obj.name in ['crack', 'corrosion'] and not obj.difficult]
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        ...
        labels = [VOC2007CrackCorrosion.CATEGORY_TO_LABEL_DICT[obj.name] for obj in annotation.objects]
        ...
        if self._mode == VOC2007CrackCorrosion.Mode.TRAIN and random.random() > 0.5:
            image = ImageOps.mirror(image)
            bboxes[:, [0, 2]] = image.width - bboxes[:, [2, 0]]  # index 0 and 2 represent `left` and `right` respectively

        image, scale = VOC2007CrackCorrosion.preprocess(image, self._image_min_side, self._image_max_side)
    ...
    def evaluate(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]) -> Tuple[float, str]:
        ...
        class_to_ap_dict = {}
        for c in range(1, VOC2007CrackCorrosion.num_classes()):
            category = VOC2007CrackCorrosion.LABEL_TO_CATEGORY_DICT[c]
            try:
                path_to_cache_dir = os.path.join('caches', 'voc2007-crack-corrosion')
        ...
        detail = ''
        for c in range(1, VOC2007CrackCorrosion.num_classes()):
            detail += '{:d}: {:s} AP = {:.4f}\n'.format(c, VOC2007CrackCorrosion.LABEL_TO_CATEGORY_DICT[c], class_to_ap_dict[c])
    ...
    def _write_results(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]):
        ...
        for c in range(1, VOC2007CrackCorrosion.num_classes()):
            class_to_txt_files_dict[c] = open(os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(VOC2007CrackCorrosion.LABEL_TO_CATEGORY_DICT[c])), 'w')
